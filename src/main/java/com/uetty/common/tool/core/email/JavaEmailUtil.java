...
public class JavaEmailUtil {
	...
	private static final Logger logger = LoggerFactory.getLogger(JavaEmailUtil.class);

	private static final String SMTP_PORT = "25";
	private static final String SMTP_SERVER = "mail.qq.com";
	private static final String SMTP_USERNAME = "abc@qq.com";
	private static final String SMTP_PASSWORD = "12345678";
	private static final boolean USE_SSL = false;
	
	private static final JavaEmailSender emailSender = new JavaEmailSender(SMTP_USERNAME, SMTP_PASSWORD, SMTP_SERVER, SMTP_PORT, USE_SSL);
	...
	public static class ResendableEmail extends MailInfo {
		...
		public static long SEND_INTERVAL = 1000 * 60;// 一分钟后重发
		...
		public boolean ready() {
			synchronized (ResendableEmail.class) {
				...
				return lastSendTime == null || lastSendTime + SEND_INTERVAL <= System.currentTimeMillis();
			}
		}

		private static void removeDuplicateAddress(List<String> addrs) {
			Set<String> set = new HashSet<>(addrs);
			addrs.clear();
			addrs.addAll(set);
		}
	...
	public static class EmailService extends Thread {
		...
		List<ResendableEmail> list = new ArrayList<>();
		...
		public void run() {
			while (true) {
				...
				try {
					...
					LockSupport.parkNanos(sleepTime * 1000_000L);
		...
		private void handException(Exception e, ResendableEmail remail) {
			if (remail == null)
				return;
			logger.error("", e);
			if (remail.rebuildEmailIfCanResend(e)) {// 三次重发机会
				synchronized (EmailService.class) {
					list.add(remail);
				}
			}
